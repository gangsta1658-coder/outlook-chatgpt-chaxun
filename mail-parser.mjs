...
export function inspectMailboxMessages(messages) {
  ...
  for (const [index, message] of list.entries()) {
    ...
    if (REWARD_SUBJECT.test(subject)) {
      const credits = extractCredits(`${subject}\n${body}`);
      if (credits != null) rewardMatches.push({ credits, receivedAt, index, subject: subject.slice(0, 180) });
    }
  }
...
function extractCredits(text) {
  CREDITS_PATTERN.lastIndex = 0;
  let best = null;
  let match;
  while ((match = CREDITS_PATTERN.exec(text))) {
    ...
    const score = /\b(?:reward|bonus|referral|received?|claim(?:ed|ing)?)\b/i.test(context) ? 2 : 0;
...
function isDeactivationMessage(value) {
  ...
  return BANNED_SUBJECT.test(text) || BANNED_REVERSED.test(text);
}
